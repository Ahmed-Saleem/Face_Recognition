...
def processing_thread(input_path, output_without_audio_path):
    # Open video 
...
# def download_youtube_chunks(url, input_dir, chunk_duration=120, total_chunks=5):
#     if not os.path.exists(input_dir):
#         os.makedirs(input_dir)

#     youtube = pytube.YouTube(url)
#     stream = youtube.streams.filter(only_audio=False, res = "1080p", file_extension = "mp4").first()

#     for chunk_index in range(total_chunks):
#         start_time = chunk_index * chunk_duration
#         end_time = (chunk_index + 1) * chunk_duration

#         chunk_filename = os.path.join(input_dir, f"chunk_{chunk_index + 1}.mp4")
#         stream.download(output_path=intput_dir, start=start_time, end=end_time, filename_prefix="tmp")

#         print(f"Downloaded chunk {chunk_index + 1} of {total_chunks}")

def download_youtube_chunks(url, input_dir, chunk_duration, total_chunks):
    ...
    for chunk_index in range(total_chunks):
        ...
        cmd = [
            ...
            "-o",
            chunk_filename,
            "--hls-duration", "60",  # Limit the duration to 4 seconds
...
def main():
    ...
    # Download chunks
    download_youtube_chunks(url, input_dir, chunk_duration, total_chunks)

    # Process each chunk
    input_paths = glob.glob(input_dir + "/*.mp4")
    ...
    for input_path in input_paths:
        ...
        # Process video
        processing_thread(input_path, output_without_audio_path)

        # Merge audio into output video
        output_path = os.path.join(output_dir, f"output_{output_index}.mp4")
