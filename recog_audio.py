...
def main():
    ...
    coverage_time = 0
    for p in person_data:
        if len(p['timestamps']) >= 20:
            p['start time'].append(p['timestamps'][0])
            for t in range(0,len(p['timestamps'])):
                ts = time_to_seconds(p['timestamps'][t])
                prev_ts = time_to_seconds(p['timestamps'][t-1])
                if ts - prev_ts >= 2:
                    p['start time'].append(p['timestamps'][t])
                    p['end time'].append(p['timestamps'][t-1])
            if len(p['start time']) != len(p['end time']):
                if len(p['start time']) > len(p['end time']):
                    p['end time'].append(p['timestamps'][-1])
                else:
                    p['end time'][:-1]
            #p['end time'].append(p['timestamps'][-1])
            #print('start',len(p['start time']))
            #print('end',len(p['end time']))
            for tt in range(0, len(p['start time'])):
                tts = (time_to_seconds(p['end time'][tt]) - time_to_seconds(p['start time'][tt]))*(-1)
                coverage_time = coverage_time + tts
            p['coverage time'] = time_str(coverage_time)
    
    # Convert the dictionary to a DataFrame
    df = pd.DataFrame(person_data)
    condition = df['timestamps'].apply(len) > 1
