...
def main():
    ...
    output_path = "testing_videos/output_Arif_Alvi2.mp4"
    # scale_factor = 0.5
    ...
    while cap.isOpened():
        ...
        for i in range(len(bboxs)):
            ...
            if name == null:
                ...
                frame_timestamp_str = datetime.fromtimestamp(frame_timestamp).strftime('%H:%M:%S')
                if score < 0.35:
                    label = "Unknown"
                    caption = label
                    ...
                    # Create a dictionary for the current person's data
                    person_entry = {
                        'thumbnail': frame[y1:y2, x1:x2].tolist(),  # Convert the image array to a nested list
                        'person name': name,
                        'timestamps': [frame_timestamp_str]
                        #'start time': [],
                        #'end time': [],  # Placeholder for now
                        #'coverage time': None
                    }
